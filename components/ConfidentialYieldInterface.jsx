...
import createFHEClient from '../utils/simple-fhe';

const ConfidentialYieldInterface = () => {
    ...
    const initializeFHE = async () => {
        ...
        const success = await client.initialize('user-private-key-123');
        if (success) {
            setFheClient(client);
            setIsConnected(true);
        }
    ...
    const handleStake = async () => {
        if (!fheClient || userBalance <= 0) return;

        setIsCalculating(true);

        try {
            // Encrypt the stake amount
            const encrypted = fheClient.encrypt(userBalance);
            ...
            const rate = fheClient.decrypt(encryptedRate);
            setYieldRate(rate);

            // Calculate encrypted yield
            const encryptedYieldAmount = fheClient.calculateEncryptedYield(encrypted, encryptedRate);
    ...
    const handleWithdraw = () => {
        if (!fheClient || !encryptedBalance) return;
    ...
    const getBalanceSummary = () => {
        if (!fheClient || !encryptedBalance || !encryptedYield) return null;
        return fheClient.getBalanceSummary(encryptedBalance, encryptedYield);
    };
...
export default ConfidentialYieldInterface;
